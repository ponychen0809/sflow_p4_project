...
class sFlowAgent:
    def __init__(self, datagram_version, address_type, agent_address, sub_agent_id, collector_address):
        ...
        self.real_total_packets = 0
        self.estimated_total_packets = 0

    def processSamples(self, ip_layer, layer4, ingress_port, egress_port, total_packets):
        self.real_total_packets = total_packets
        self.estimated_total_packets += self.sampling_rate
        uptime = int((time.time() - self.start_time) * 1000)
        with open("statistics.txt", "w") as file:
            file.write("{}\n".format(self.real_total_packets))
            file.write("{}\n".format(self.estimated_total_packets))
            file.write("{}\n".format(uptime))
